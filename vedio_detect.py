...
def init_feature(cap): #初始化特徵點
    ...
    mask1[10:250, x-100:y] = 255 #設定特徵取樣範圍
...
def tracert_feature(cap,feature_params,lk_params,color,old_gray,p0,mask):
    ...
    while(1):
        ...
        for i,(new,old) in enumerate(zip(good_new,good_old)):
            ...
            #print(b-d)
            count_i +=1
            total_data_x = total_data_x + (a-c)
            total_data_y = total_data_y + (b-d)
        ...
        if (total_data_x/count_i) > 0.05 and (total_data_y/count_i) < 0.1 and not st is None:
            print('Move forward')
            cv2.putText(img,'Move forward',(500, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255), 2, cv2.LINE_AA,)
            
        if (total_data_x/count_i) < 0.1 and (total_data_y/count_i) < 0.1 and not st is None and (total_data_x/count_i) != 0:
            print('Stop Move')
            cv2.putText(img,'Stop Move',(500, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255), 2, cv2.LINE_AA,)
            print(str((total_data_x/count_i)))
...
if __name__ == '__main__':
    ...
    cap = cv2.VideoCapture('./2021-10-26_13-55-13-trim.mp4')
    feature_params,lk_params,color,old_gray,p0,mask = init_feature(cap)
    tracert_feature(cap,feature_params,lk_params,color,old_gray,p0,mask)    
