...
points = [[100, 0], [300, 0], [300, 70], [100, 70]]
...
def init_feature(points,old_frame): #初始化特徵點
    #points = [[100, 2], [300, 2], [300, 50], [100, 50]]
    #設定特徵取樣參數
    ...
    #old_frame = cv2.imread("./frame/frame.png") #讀取照片
    ...
    #print(p0)
    #cv2.imshow("old_gray", old_gray) #顯示遮罩後照片    
    return feature_params,lk_params,color,old_gray,p0

def tracert_feature(feature_params,lk_params,color,old_gray,p0,points,frame2):
    #cv2.imshow("frame2", frame2) #顯示遮罩後照片
    dst=croppoly(frame2,points)
    frame_gray = cv2.cvtColor(dst, cv2.COLOR_BGR2GRAY)
    #cv2.imshow("frame_gray", frame_gray) #顯示遮罩後照片
    #print(p0)
    ...
    if st is None: #特徵點已移出畫面, 重新擷取特徵點
        feature_params,lk_params,color,old_gray,p0 = init_feature(points,frame2)            
        old_gray,p0 = tracert_feature(feature_params,lk_params,color,old_gray,p0,points,frame2)
    else:
        good_new = p1[st==1]
        good_old = p0[st==1]
        ...
        for i,(new,old) in enumerate(zip(good_new,good_old)):
            ...
            dst = cv2.circle(dst,(a,b),5,color[i].tolist(),-1) #畫特徵點
        ...
        if (x_mean) < 0.05 and x_mean > -0.05 and not st is None:
            cv2.putText(img,'Stop Move',(5, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255), 1, cv2.LINE_AA,)
            print('x value:'+str(x_mean))
            print('y value:'+str(y_mean))          
            
        else:
            print('Move forward')
            cv2.putText(img,'Move forward',(5, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255), 1, cv2.LINE_AA,)
            print('x value:'+str(x_mean))
    
    
    
            #if (y_mean) < -1  and (x_mean) > 5 and not st is None:
            if (x_mean) > 9 and not st is None:
                print('Turn left')                
                cv2.putText(img,'Turn left',(5, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255), 1, cv2.LINE_AA,)
                print('y value:'+str(y_mean))
                
            #if (y_mean) > -1 and (x_mean) < -5 and not st is None:
            if (x_mean) < -5 and not st is None:
                print('Turn right')                
                cv2.putText(img,'Turn right',(5, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255), 1, cv2.LINE_AA,)
                print('y value:'+str(y_mean))
                
            
            
        cv2.putText(img,'value1:'+str((x_mean)) ,(20, 90), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255), 1, cv2.LINE_AA,)
        cv2.putText(img,'value2:'+str((y_mean)) ,(20, 120), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255), 1, cv2.LINE_AA,)
...
while True:
    ...
    if init_flag ==0:
        feature_params,lk_params,color,old_gray,p0 = init_feature(points,frame1)
        init_flag =1
    else:
        old_gray,p0 = tracert_feature(feature_params,lk_params,color,old_gray,p0,points,frame1)
